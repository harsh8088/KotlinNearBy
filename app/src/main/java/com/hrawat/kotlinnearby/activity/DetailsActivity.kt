...
class DetailsActivity : AppCompatActivity(), OnMapReadyCallback {
    companion object {
        const val BUNDLE_EXTRA_PLACE = "BUNDLE_EXTRA_PLACE"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle != null && bundle.containsKey(BUNDLE_EXTRA_PLACE)) {
            val gson = Gson()
            placeResultModel =
                gson.fromJson(bundle.getString(BUNDLE_EXTRA_PLACE), PlaceResultModel::class.java)
        }
        init()
    }
    ...
    private fun init() {
        ...
        if (placeResultModel.opening_hours.open_now)
            imageViewStatus.background = ContextCompat.getDrawable(baseContext, R.drawable.ic_open)
        else
            imageViewStatus.background =
                ContextCompat.getDrawable(baseContext, R.drawable.ic_closed)
    }
